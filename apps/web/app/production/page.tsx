      ...
      {inProduction.length > 0 && (
        <section className="mb-10">
          ...
          <div className="bg-white border rounded-xl p-6">
            {/* For simplicity, show timeline for the first in-production campaign */}
            <ProductionTimeline campaign={inProduction[0]} />
          </div>
        </section>
      )}
          ...
          <div className="space-y-3">
            {ready.map((campaign) => (
              <div key={campaign.id} className="flex items-center justify-between border rounded-lg p-4 bg-white">
                ...
                <button
                  onClick={() => {
                    // For now still using old mutation for ready campaigns
                    // In future we should route through checkout here too
